...
taxa$asv_number <- c(1:dim(taxa)[1])
...
# sequence metadata
mdatseq <- read.csv("raw-data/master-dataset-13c_ITS_16S.csv")
# set groupings
...
mdatseq$file.names <- mdatseq$file.names.y
...
#mdatseq <- mdatseq[-which(is.na(mdatseq$richness)==TRUE),] 
...
# ##### community visualization with NMDS:
# 
# # All data together
# ssn <- c("24 hr", "6 mo", NA)
# divmat_pa_2 <- divmat_pa[which(mdatseq$Harvest.day2 %in% ssn),]
# mdatseq_2 <- mdatseq[which(mdatseq$Harvest.day2 %in% ssn),]
# #rem <- c("318_Sum_2021", "220_Fall_2021")
# m <- vegan::metaMDS(divmat_pa_2, display="sites", wascores="TRUE", maxit=5000)
...
# cor(distance,ax1_dist)^2 # 0.9157512
# ax2_dist <- vegdist(m$points[,2],method="euclidean")
# cor(distance,ax2_dist)^2 # 0.2967548
# with(mdatseq, levels(Harvest.day2))
# scl <- 2
# # color by residue type
# png(paste0("figures/*communities/nmds/16S_all_cover.png"), width=600, height=650, res = 150)
# plot(s$sites, type="n")#, xlim=c(-1,2.5), ylim=c(-1.1,1.5))
# #with(mdatseq_2, points(s$sites,  scaling =scl, pch =21, bg = alpha(covercols[Residue.type], 0.9)))
# with(mdatseq_2, points(s$sites,  scaling =scl, pch =c(21,23)[Harvest.day2], bg = alpha(covercols[Residue.type],0.7)))
# #with(mdatseq, legend("topright", legend=levels(Cover), bty="n", col="black", pch=21, pt.bg=alpha(covercols,0.9)))
# ordiellipse(m, group=mdatseq$Residue.type, col = alpha(covercols, 0.9), label=F, draw="polygon", border = NA )
# legend("top", legend=levels(mdatseq_2$Residue.type),fill=covercols, cex=0.8)
# legend("bottom", legend=levels(mdatseq_2$Harvest.day2),pch =c(21,23), cex=0.8)
# dev.off()
# 
# 
# # 24 hr
...
# distance <- vegdist(divmat_pa_2, method="bray")
# ax1_dist <- vegdist(m$points[,1],method="euclidean")
# cor(distance,ax1_dist)^2 # 0.9508669
# ax2_dist <- vegdist(m$points[,2],method="euclidean")
# cor(distance,ax2_dist)^2 # 0.2770265
# with(mdatseq, levels(Harvest.day2))
# scl <- 2
# # color by residue type
# png(paste0("figures/*communities/nmds/16S_24hr_res.png"), width=600, height=650, res = 150)
# plot(s$sites, type="n")#, xlim=c(-1,2.5), ylim=c(-1.1,1.5))
# with(mdatseq_2, points(s$sites,  scaling =scl, pch =c(21,23)[Moisture.treatment], bg = alpha(covercols[Residue.type],0.7)))
# #with(mdatseq, legend("topright", legend=levels(Cover), bty="n", col="black", pch=21, pt.bg=alpha(covercols,0.9)))
# ordiellipse(m, group=mdatseq_2$Residue.type, col = alpha(covercols, 0.9), label=F, draw="polygon", border = NA )
# legend("top", legend=levels(mdatseq_2$Residue.type),fill=covercols, cex=0.8)
# legend("bottom", legend=levels(mdatseq_2$Moisture.treatment),pch =c(21,23), cex=0.8)
# dev.off()
# 
# 
# # 6 mo
...
# distance <- vegdist(divmat_pa_2, method="bray")
# ax1_dist <- vegdist(m$points[,1],method="euclidean")
# cor(distance,ax1_dist)^2 # 0.9530642
# ax2_dist <- vegdist(m$points[,2],method="euclidean")
# cor(distance,ax2_dist)^2 # 0.1684401
# with(mdatseq, levels(Harvest.day2))
# scl <- 2
# # color by residue type
# png(paste0("figures/*communities/nmds/16S_6mo_res.png"), width=600, height=650, res = 150)
# plot(s$sites, type="n")#, xlim=c(-1,2.5), ylim=c(-1.1,1.5))
# with(mdatseq_2, points(s$sites,  scaling =scl, pch =c(21,23)[Moisture.treatment], bg = alpha(covercols[Residue.type],0.7)))
# #with(mdatseq, legend("topright", legend=levels(Cover), bty="n", col="black", pch=21, pt.bg=alpha(covercols,0.9)))
# ordiellipse(m, group=mdatseq_2$Residue.type, col = alpha(covercols, 0.9), label=F, draw="polygon", border = NA )
...
write.csv(permmod2, paste0("Model-output/permanova/with glucose_16S by timepoint_24 hr.csv"))
...
permmod <- adonis2(divmat_pa_6mo ~ Moisture.treatment*Residue.type*Glucose.addition.treatment, 
                   ...
                   by = "terms" )
permmod2 <- round(as.data.frame(permmod), 3)
write.csv(permmod2, paste0("Model-output/permanova/with glucose_16S by timepoint_6 mo.csv"))
...
write.csv(mod.effects, paste0("Model-output/permanova/16S by timepoint.csv"))
...
### do soil properties influence microbial community composition?
...
# res <- cor(mdatseq_24hr[,c("CUE", "X13maoc", "X13poc", "X13co2", "X13mbc",
...
myrda_24hr0 <- capscale(formula = divmat_pa_24hr ~CUE+ X13maoc+ X13poc+ X13co2+ X13mbc+ 
                          X13doc+ X13soc+ co2.native+
                          pom.c.g.kg+  pom.cn+ #pom.n.g.kg+
                          maom.c.g.kg+ maom.cn+ #maom.n.g.kg+ 
                          bulk.soil.C.g.kg+ #bulk.soil.cn+  #bulk.soil.N.g.kg+ 
...
#myrda_24hr <- ordistep(myrda_24hr0, perm.max = 10) 
myrda_24hr <- capscale(formula = divmat_pa_24hr ~CUE+ X13co2+ X13mbc+co2 +maom.cn +bulk.soil.C.g.kg,
                        distance = "bray", 
                        data = mdatseq_24hr, na.action = "na.omit")
...
write.csv(as.data.frame(summary(myrda_24hr)$biplot), paste0("Model-output/db-rda/16S_24hr_loadings.csv"))
# site coordinates
write.csv(as.data.frame(summary(myrda_24hr)$site), paste0("Model-output/db-rda/16S_24hr_axes.csv"))
# variation explained by first two axes
write.csv(summary(myrda_24hr)$cont$importance, paste0("Model-output/db-rda/16S_24hr_variance-explained.csv"))
# variation explained by each soil property
mdatseq_24hr_nonas0 <- na.omit(mdatseq_24hr[,c("Residue.type", "Moisture.treatment", "Glucose.addition.treatment", "file.names.y",
...
write.csv(round(cbind(ef$vectors$arrows, ef$vectors$r),2), paste0("Model-output/db-rda/16S_24hr_variance-by-predictors.csv"))
...
summary(myrda_24hr)$cont$importance[2,1] #0.3829738
summary(myrda_24hr)$cont$importance[2,2] #0.03544864
db24hr <- ggplot(rdadat, aes(CAP1, CAP2)) +
  ...
  labs(x=("RDA 1 (38.3%)"), y="RDA 2 (3.6%)") + # amend according to variance explained
  lims(x=c(-1.2,1.5)) + 
  ...
  theme()
db24hr
pdf(paste0("Figures/*communities/db-rda/16S_24hr1.pdf"), height=3.75, width=5.5)
db24hr
dev.off()
...
# res <- cor(mdatseq_6mo[,c("CUE", "X13maoc", "X13poc", "X13co2", "X13mbc",
#                           "X13doc", "X13soc", "co2.native",
#                           "pom.c.g.kg",  "pom.cn", "pom.n.g.kg",
#                           "maom.c.g.kg", "maom.cn", "maom.n.g.kg",
#                           "bulk.soil.C.g.kg", "bulk.soil.cn",  "bulk.soil.N.g.kg",
...
myrda_6mo0 <- capscale(formula = divmat_pa_6mo ~CUE+ X13maoc+ X13poc+ X13co2+ #X13mbc+ 
                        X13doc+ X13soc+ co2.native+
                        pom.c.g.kg+  pom.cn+ #pom.n.g.kg+
                        maom.c.g.kg+ maom.cn+ #maom.n.g.kg+ 
                        bulk.soil.C.g.kg+ #bulk.soil.cn+  #bulk.soil.N.g.kg+ 
...
#myrda_6mo <- ordistep(myrda_6mo0, perm.max = 10) 
myrda_6mo <- capscale(formula = divmat_pa_6mo ~co2+ bulk.soil.C.g.kg+ maom.cn,
                       distance = "bray", 
                       data = mdatseq_6mo, na.action = "na.omit")
...
# export mod effects
write.csv(t(mod.effects), paste0("Model-output/db-rda/16S_dbrda.csv"))

## rda all
# factors associated with each axis
write.csv(as.data.frame(summary(myrda_6mo)$biplot), paste0("Model-output/db-rda/16S_6mo_loadings.csv"))
# site coordinates
write.csv(as.data.frame(summary(myrda_6mo)$site), paste0("Model-output/db-rda/16S_6mo_axes.csv"))
# variation explained by first two axes
write.csv(summary(myrda_6mo)$cont$importance, paste0("Model-output/db-rda/16S_6mo_variance-explained.csv"))
# variation explained by each soil property
mdatseq_6mo_nonas0 <- na.omit(mdatseq_6mo[,c("Residue.type", "Moisture.treatment", "Glucose.addition.treatment", "CUE", "X13maoc", "X13poc", "X13co2", #"X13mbc", 
...
ef <- envfit(myrda_6mo, mdatseq_6mo_nonas,
             choices = c(1,2), permutations = 0, na.rm=TRUE)
write.csv(round(cbind(ef$vectors$arrows, ef$vectors$r),2), paste0("Model-output/db-rda/16S_6mo_variance-by-predictors.csv"))
...
rdadat$Glucose.addition.treatment <- as.factor(rdadat$Glucose.addition.treatment)
levels(rdadat$Glucose.addition.treatment) <- c("No glucose", "Glucose")
summary(myrda_6mo)$cont$importance[2,1] #0.4594538
summary(myrda_6mo)$cont$importance[2,2] #0.02555805
db6mo <- ggplot(rdadat, aes(CAP1, CAP2)) +
  geom_point(aes(shape=Moisture.treatment, color=Residue.type, alpha=Glucose.addition.treatment)) +
  theme_bw() +
  scale_color_manual(values = covercols) +
  scale_alpha_manual(values = c(0.5,1)) +
  labs(x=("RDA 1 (46.0%)"), y="RDA 2 (2.6%)") + # amend according to variance explained
  lims(x=c(-1.35,1.5)) + guides(shape=guide_legend(title="Moisture treatment"), color=guide_legend(title="Residue type"), alpha=guide_legend(title="Glucose addition")) +
  geom_segment(data=df3, aes(x=0, xend=CAP1, y=0, yend=CAP2), color="black", arrow=arrow(length=unit(0.01,"npc"))) +
  geom_text(inherit.aes=FALSE, data=df3, x=df3$CAP1,y=df3$CAP2,label=labs2, 
            hjust=0.5*(1-sign(df3$CAP1)),vjust=0.5*(1-sign(df3$CAP2)), 
            color="blue", size=2)
...
db6mo
pdf(paste0("Figures/*communities/db-rda/16S_6mo1.pdf"), height=3.75, width=5.5)
db6mo
dev.off()


ann1 <- ggplot() + 
  geom_text(aes(x=0, y=0, label ="C) Bacterial community at 24 hr"), size = 4, hjust = 0.5) +
  theme_void()

ann2 <- ggplot() + 
  geom_text(aes(x=0, y=0, label ="D) Bacterial community at 6 mo"), size = 4, hjust = 0.54) +
  theme_void()
...
#figure
png("figures/*communities/db-rda/16S.png", 
...
save.image(file='code/3d_communityanalysis_16S.RData')
load(file='code/3d_communityanalysis_16S.RData')
...
data.frame(rownames(abundances), mdatseq$file.names.y)
# samples to remove
...
taxa2$Kingdom <- paste0("k__", taxa2$Kingdom)
taxa2$Kingdom <- gsub(taxa2$Kingdom, pattern="k__NA", replacement="NA")
taxa2$Phylum <- paste0("p__", taxa2$Phylum)
taxa2$Phylum <- gsub(taxa2$Phylum, pattern="p__NA", replacement="NA")
taxa2$Class <- paste0("c__", taxa2$Class)
taxa2$Class <- gsub(taxa2$Class, pattern="c__NA", replacement="NA")
taxa2$Order <- paste0("o__", taxa2$Order)
taxa2$Order <- gsub(taxa2$Order, pattern="o__NA", replacement="NA")
taxa2$Family <- paste0("f__", taxa2$Family)
taxa2$Family <- gsub(taxa2$Family, pattern="f__NA", replacement="NA")
...
write.csv(abundances_t, "raw-data/sequence/16S/Abundances by taxon_24hr.csv")
...
dd$taxon <- paste(dd$genus) # dd$taxon <- paste(dd$genus, dd$species, sep=" ")
...
dd <- dd[order(dd$kingdom, dd$phylum, dd$class, dd$order, dd$family, dd$genus),]

# export lefse results 
write.csv(dd, "raw-data/sequence/16S/Lefse_24hr.csv")

# export  abundances for lefse taxa 
abundances_t_lefse <- abundances_t[which(rownames(abundances_t) %in% dd$features),]
write.csv(abundances_t_lefse, "raw-data/sequence/16S/Abundances by taxon_24hr_Lefse.csv")
...
ddgrid$kingdom <- rep(NA, dim(ddgrid)[1])
ddgrid$phylum <- rep(NA, dim(ddgrid)[1])
ddgrid$class <- rep(NA, dim(ddgrid)[1])
ddgrid$order <- rep(NA, dim(ddgrid)[1])
ddgrid$family <- rep(NA, dim(ddgrid)[1])
ddgrid$genus <- rep(NA, dim(ddgrid)[1])
for(i in 1:dim(ddgrid)[1]){
  ...
  kingdom <-  dd$kingdom[which(dd$taxon==ddgrid$taxon[i])]
  ddgrid$kingdom[i] <- ifelse(length(kingdom)==0, NA, kingdom)
  phylum <-  dd$phylum[which(dd$taxon==ddgrid$taxon[i])]
  ddgrid$phylum[i] <- ifelse(length(phylum)==0, NA, phylum)
  class <-  dd$class[which(dd$taxon==ddgrid$taxon[i])]
  ddgrid$class[i] <- ifelse(length(class)==0, NA, class)
  order <-  dd$order[which(dd$taxon==ddgrid$taxon[i])]
  ddgrid$order[i] <- ifelse(length(order)==0, NA, order)
  family <-  dd$family[which(dd$taxon==ddgrid$taxon[i])]
  ddgrid$family[i] <- ifelse(length(family)==0, NA, family)
  genus <-  dd$genus[which(dd$taxon==ddgrid$taxon[i])]
  ddgrid$genus[i] <- ifelse(length(genus)==0, NA, genus)
...
#ddgrid$primarylifestyle <- factor(ddgrid$primarylifestyle, levels(ddgrid$primarylifestyle)[rev(c(2,4,6,8,9,1,3,5,7))])
ddgrid1 <- ddgrid[order(ddgrid$kingdom, ddgrid$phylum, ddgrid$class, ddgrid$order, ddgrid$family, ddgrid$genus),]
ddgrid1$taxon <- as.factor(ddgrid1$taxon)
ddgrid1$taxon <- factor(ddgrid1$taxon, unique(ddgrid1$taxon))
ddgrid1$taxon <- factor(ddgrid1$taxon, levels(ddgrid1$taxon)[c(1:93,95:229,94,230)]) # candidate division WPS-1 is out of order
...
lab <- paste0(gsub(result$phylum, pattern="p__", replacement=""))
segs <- data.frame(x=5.5, xend=0.5,
                   y=result$cumulative+0.5, yend=result$cumulative+0.5,
                   label=gsub(lab, pattern=" ", replacement="\n"),
                   ylab=result$cumulativehal)
segs$ybar1 <- c(0.5, segs$yend[c(1:dim(segs)[1]-1)]+0.5)
segs$ybar2 <- c(segs$y-0.5)
segs$label[14] <- "WPS-1" # rename WPS-1
segs$label[15] <- "" # rename NA

ann1 <- ggplot() + 
  geom_text(aes(x=0.1, y=0, label ="A) Bacterial taxa at 24 hr"), size = 4, hjust = 0.5) +
...
ddgrid1 <- ddgrid1[order(ddgrid1$kingdom, ddgrid1$phylum, ddgrid1$class, ddgrid1$order, ddgrid1$family, ddgrid1$genus),]

lefsep <-ggplot(ddgrid1, aes(fill= score)) + 
  geom_tile(color="black", aes(y=(ddgrid1$taxon), x=Residue.type)) + 
  labs(y="")+
  ...
  #scale_fill_gradient2(guide="colorbar",na.value="white", lim16S = c(-1, 1), low="orchid2", high="seagreen3", mid="white") +
  theme(axis.text.x = element_text(angle = 270, vjust = 0, hjust=0), 
        axis.text.y = element_text(size=3, face="italic"), 
        plot.margin = unit(c(0.1, 0.2, 0.1, 2), "cm"),
        axis.title.x=element_blank(), legend.position="top", 
        legend.text=element_text(size=7), legend.key.size = unit(0.45, 'cm'), legend.title=element_text(size=9)) +
  geom_segment(inherit.aes=FALSE, size=0.5, data=segs, aes(x=x, y=y, yend=yend, xend=xend)) +
  coord_cartesian(xlim=c(1,5), clip="off")+
  geom_segment(inherit.aes=FALSE, size=0.5, data=segs, aes(x=-4, xend=-4, y=ybar1, yend=ybar2)) +
  annotate("text", x = -4.1, y = segs$ylab+0.2, label = segs$label, size=2.3, hjust=1, vjust=0.5) #+
#theme_minimal()
...
figure <- ggpubr::ggarrange(ann1, lefsep, legend="top",
                            ncol = 1, nrow = 2, heights=c(0.025,0.975)) 
#figure
png("figures/*lefse/1_16S_24 hr.png", 
    width = 1000, height =4000, res=400, units="px")
figure
dev.off()
...
abundances_t_lefse <- read.csv("raw-data/sequence/16S/Abundances by taxon_24hr_Lefse.csv")
...
abundances_t_lefse$taxon <- paste(abundances_t_lefse$genus)
...
abundances_t_lefse <- abundances_t_lefse[order(abundances_t_lefse$kingdom, abundances_t_lefse$phylum, abundances_t_lefse$class, abundances_t_lefse$order, abundances_t_lefse$family, abundances_t_lefse$genus),]
abundances_t_lefse$taxon <- as.factor(abundances_t_lefse$taxon)
abundances_t_lefse$taxon <- factor(abundances_t_lefse$taxon, unique(abundances_t_lefse$taxon))
#ddgrid1$taxon <- factor(ddgrid1$taxon, levels(ddgrid1$taxon)[c(1:93,95:229,94,230)]) # candidate division WPS-1 is out of order
#abundances_t_lefse$taxon %in% dd$taxon
...
mdatseq[which(mdatseq$Glucose.addition.treatment=="G-00"),c("X13soc", "X13poc", "X13maoc", "X13doc", "X13mbc", "X13co2", "CUE", "co2.native")] <- mdatseq[which(mdatseq$Glucose.addition.treatment=="G-50"),c("X13soc", "X13poc", "X13maoc", "X13doc", "X13mbc", "X13co2", "CUE", "co2.native")]
# order by sequence file names
mdatseq <- mdatseq[order(mdatseq$file.names.y),]
...
abundances_t_lefse_t2 <- abundances_t_lefse_t[rownames(abundances_t_lefse_t) %in% mdatseq_24hr$file.names.y,]
abundances_t_lefse_t2$file.names.y <- rownames(abundances_t_lefse_t2)
unique(mdatseq_24hr$file.names.y==abundances_t_lefse_t2$file.names.y) #check

# combine abundances and soil property data
newdat <- merge(mdatseq_24hr, abundances_t_lefse_t2, by="file.names.y",
                all.x = TRUE, all.y = TRUE)

terms.soil <- c(rownames(summary(myrda_24hr)$biplot)) 
terms.taxa <- colnames(abundances_t_lefse_t2)
terms.taxa <- terms.taxa[-which(terms.taxa %in% c("file.names.y", "file.names"))]  
...
ddgrid2$kingdom <- rep(NA, dim(ddgrid2)[1])
ddgrid2$phylum <- rep(NA, dim(ddgrid2)[1])
ddgrid2$class <- rep(NA, dim(ddgrid2)[1])
ddgrid2$order <- rep(NA, dim(ddgrid2)[1])
ddgrid2$family <- rep(NA, dim(ddgrid2)[1])
ddgrid2$genus <- rep(NA, dim(ddgrid2)[1])
for(i in 1:dim(ddgrid2)[1]){
  ...
  kingdom <-  dd$kingdom[which(dd$taxon==ddgrid2$taxon[i])]
  ddgrid2$kingdom[i] <- ifelse(length(kingdom)==0, NA, kingdom)
  phylum <-  dd$phylum[which(dd$taxon==ddgrid2$taxon[i])]
  ddgrid2$phylum[i] <- ifelse(length(phylum)==0, NA, phylum)
  class <-  dd$class[which(dd$taxon==ddgrid2$taxon[i])]
  ddgrid2$class[i] <- ifelse(length(class)==0, NA, class)
  order <-  dd$order[which(dd$taxon==ddgrid2$taxon[i])]
  ddgrid2$order[i] <- ifelse(length(order)==0, NA, order)
  family <-  dd$family[which(dd$taxon==ddgrid2$taxon[i])]
  ddgrid2$family[i] <- ifelse(length(family)==0, NA, family)
  genus <-  dd$genus[which(dd$taxon==ddgrid2$taxon[i])]
  ddgrid2$genus[i] <- ifelse(length(genus)==0, NA, genus)
}
...
#ddgrid2$primarylifestyle <- factor(ddgrid2$primarylifestyle, levels(ddgrid2$primarylifestyle)[rev(c(2,4,6,8,9,1,3,5,7))])
...
ddgrid3 <- ddgrid2[order(ddgrid2$kingdom, ddgrid2$phylum, ddgrid2$class, ddgrid2$order, ddgrid2$family, ddgrid2$genus),]
ddgrid3$taxon <- as.factor(ddgrid3$taxon)
ddgrid3$taxon <- factor(ddgrid3$taxon, levels=unique(ddgrid3$taxon))
ddgrid3$taxon <- factor(ddgrid3$taxon, levels(ddgrid3$taxon)[c(1:93,95:229,94,230)]) # candidate division WPS-1 is out of order
...
segs2 <- data.frame(x=length(unique(ddgrid3$soil))+0.5, xend=0,
                    y=result$cumulative+0.5, yend=result$cumulative+0.5,
                    label=paste0(gsub(result$phylum, pattern="p__", replacement="")))

ann2 <- ggplot() + 
  geom_text(aes(x=0.1, y=0, label =""), size = 4, hjust = 0.5) +
  theme_void()

labs2 <- c(expression(paste("MAOM C:N")), 
           #expression(paste("MAO"^"13","C")),
           #expression(paste("PO"^"13","C")),
           expression(paste("SOC")),  
           expression(paste("MB"^"13","C")),
           ...
           #expression(paste("MAOM-C")), 
           #expression(paste("DOC")), 
           expression(paste("CUE")),
           expression(paste("Cumulative CO"[2])))

p<-ggplot(ddgrid3, aes(fill= cor)) + 
  geom_tile(color="black", aes(y=(ddgrid3$taxon), x=soil)) + 
  labs(y="")+
  ...
  geom_segment(inherit.aes=FALSE, size=0.5, data=segs2, aes(x=x, y=y, yend=yend, xend=xend)) #+
...
figure <- ggpubr::ggarrange(ann2,  p, 
                            ncol = 1, nrow = 2, heights=c(0.025,0.975)) 
#figure
png("figures/*lefse/1_16S_24 hr_correlations.png", 
    width = 500, height =4000, res=300, units="px")
figure
dev.off()
...
abundances_t <- abundances_t4
# export abundances by taxon
write.csv(abundances_t, "raw-data/sequence/16S/Abundances by taxon_6mo.csv")
...
# export lefse results 
write.csv(dd, "raw-data/sequence/16S/Lefse_6mo.csv")

# export  abundances for lefse taxa 
abundances_t_lefse <- abundances_t[which(rownames(abundances_t) %in% dd$features),]
write.csv(abundances_t_lefse, "raw-data/sequence/16S/Abundances by taxon_6mo_Lefse.csv")
...
ddgrid1$taxon <- as.factor(ddgrid1$taxon)
ddgrid1$taxon <- factor(ddgrid1$taxon, unique(ddgrid1$taxon))
ddgrid1$taxon <- factor(ddgrid1$taxon, levels(ddgrid1$taxon)[c(1:72,74:187,73,188)]) # candidate division WPS-1 is out of order
...
segs$ybar1 <- c(0.5, segs$yend[c(1:dim(segs)[1]-1)]+0.5)
segs$ybar2 <- c(segs$y-0.5)
segs$label[13] <- "WPS-1" # rename WPS-1
segs$label[14] <- "" # rename NA

ann1 <- ggplot() + 
  geom_text(aes(x=0.1, y=0, label ="B) Bacterial taxa at 6 mo"), size = 4, hjust = 0.5) +
...
figure <- ggpubr::ggarrange(ann1, lefsep, legend="top",
                            ncol = 1, nrow = 2, heights=c(0.025,0.975)) 
#figure
png("figures/*lefse/1_16S_6 mo.png", 
    width = 1000, height =4000, units="px", res=400)
figure
dev.off()
...
# import  abundances for lefse taxa 
abundances_t_lefse <- read.csv("raw-data/sequence/16S/Abundances by taxon_6mo_Lefse.csv")
# order by primary lifestyle
# create columns for taxonomic group
abundances_t_lefse$kingdom <- rep(NA, dim(abundances_t_lefse)[1])
abundances_t_lefse$phylum <- rep(NA, dim(abundances_t_lefse)[1])
...
abundances_t_lefse_t <- as.data.frame(t(abundances_t_lefse))
colnames(abundances_t_lefse_t) <- abundances_t_lefse_t[dim(abundances_t_lefse_t)[1],]
abundances_t_lefse_t2 <- abundances_t_lefse_t[rownames(abundances_t_lefse_t) %in% mdatseq_6mo$file.names.y,]
abundances_t_lefse_t2$file.names.y <- rownames(abundances_t_lefse_t2)
unique(mdatseq_6mo$file.names.y==abundances_t_lefse_t2$file.names.y) #check

# combine abundances and soil property data
newdat <- merge(mdatseq_6mo, abundances_t_lefse_t2, by="file.names.y",
                all.x = TRUE, all.y = TRUE)
...
ddgrid3$taxon <- as.factor(ddgrid3$taxon)
ddgrid3$taxon <- factor(ddgrid3$taxon, levels=unique(ddgrid3$taxon))
ddgrid3$taxon <- factor(ddgrid3$taxon, levels(ddgrid3$taxon)[c(1:72,74:187,73,188)]) # candidate division WPS-1 is out of order
...
figure <- ggpubr::ggarrange(ann2,  p, 
                            ncol = 1, nrow = 2, heights=c(0.025,0.975)) 
#figure
png("figures/*lefse/1_16S_6 mo_correlations.png", 
    width = 400, height =4000, res=300, units="px")
figure
dev.off()
