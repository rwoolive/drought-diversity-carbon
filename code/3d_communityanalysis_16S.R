...
# asv abundances
abundances <- dplyr::select(taxabund, contains("Wooliver_16S"))
...
# sequence metadata
mdatseq <- read.csv("raw-data/master-dataset-13c_ITS_16S_rarefied.csv")
# set groupings
...
mdatseq$file.names <- mdatseq$file.names.16S
...
write.csv(permmod2, paste0("Model-output/permanova/with glucose_16S by timepoint_24 hr_rarefied.csv"))
...
permmod <- adonis2(divmat_pa_6mo ~ Moisture.treatment*Residue.type*Glucose.addition.treatment, 
                   ...
                   by = "terms" )
permmod2 <- round(as.data.frame(permmod), 3)
write.csv(permmod2, paste0("Model-output/permanova/with glucose_16S by timepoint_6 mo_rarefied.csv"))
...
write.csv(mod.effects, paste0("Model-output/permanova/16S by timepoint_rarefied.csv"))
...
cdat <- data.frame(div = rownames(divmat_pa_24hr), mdat = mdatseq_24hr$file.names.16S)

# res <- cor(mdatseq_24hr[,c("CSE", "X13residue", "X13maoc", "X13poc", "X13co2", "X13mbc",
...
myrda_24hr <- capscale(formula = divmat_pa_24hr ~ X13residue + X13mbc  + maom.cn + bulk.soil.C.g.kg + co2, # + X13soc#CSE+ X13co2+ co2 +maom.cn +bulk.soil.C.g.kg, #X13mbc+
                        distance = "bray", 
                        data = mdatseq_24hr, na.action = "na.omit")
...
write.csv(as.data.frame(summary(myrda_24hr)$biplot), paste0("Model-output/db-rda/16S_24hr_loadings_rarefied.csv"))
# site coordinates
write.csv(as.data.frame(summary(myrda_24hr)$site), paste0("Model-output/db-rda/16S_24hr_axes_rarefied.csv"))
# variation explained by first two axes
write.csv(summary(myrda_24hr)$cont$importance, paste0("Model-output/db-rda/16S_24hr_variance-explained_rarefied.csv"))
# variation explained by each soil property
mdatseq_24hr_nonas0 <- na.omit(mdatseq_24hr[,c("Residue.type", "Moisture.treatment", "Glucose.addition.treatment", "file.names.16S",
...
write.csv(round(cbind(ef$vectors$arrows, ef$vectors$r),2), paste0("Model-output/db-rda/16S_24hr_variance-by-predictors_rarefied.csv"))
...
rdadat$file.names <- mdatseq_24hr_nonas0$file.names.16S
...
labs2 <- c(#expression(paste("CSE")),
  ...
  expression(paste("Microbial residue-"^"13", C)),
...
summary(myrda_24hr)$cont$importance[2,1] #0.4119932
summary(myrda_24hr)$cont$importance[2,2] #0.03506704
data.frame(covercols, levels(rdadat$Residue.type))
db24hr <- ggplot(rdadat, aes(CAP1, CAP2)) +
  ...
  labs(x=("RDA 1 (41.2%)"), y="RDA 2 (3.5%)") + # amend according to variance explained
  lims(x=c(-1.2,1.5)) + 
  ...
  theme()
db24hr
pdf(paste0("Figures/_communities/db-rda/16S_24hr1_rarefied.pdf"), height=3.75, width=5.5)
db24hr
dev.off()
...
# find NAs in data
drop <- mdatseq_6mo$file.names.16S[which(is.na(mdatseq_6mo$CSE))]

# drop from both dfs
divmat_pa_6mo <- divmat_pa_6mo[-which(rownames(divmat_pa_6mo) %in% drop),]
mdatseq_6mo <- mdatseq_6mo[-which(mdatseq_6mo$file.names.16S %in% drop),]
...
# export mod effects
write.csv(t(mod.effects), paste0("Model-output/db-rda/16S_dbrda_rarefied.csv"))

## rda all
# factors associated with each axis
write.csv(as.data.frame(summary(myrda_6mo)$biplot), paste0("Model-output/db-rda/16S_6mo_loadings_rarefied.csv"))
# site coordinates
write.csv(as.data.frame(summary(myrda_6mo)$site), paste0("Model-output/db-rda/16S_6mo_axes_rarefied.csv"))
# variation explained by first two axes
write.csv(summary(myrda_6mo)$cont$importance, paste0("Model-output/db-rda/16S_6mo_variance-explained_rarefied.csv"))
# variation explained by each soil property
mdatseq_6mo_nonas0 <- na.omit(mdatseq_6mo[,c("Residue.type", "Moisture.treatment", "Glucose.addition.treatment", "file.names.16S",  "CSE", "X13maoc", "X13poc", "X13co2", #"X13mbc", 
...
ef <- envfit(myrda_6mo, mdatseq_6mo_nonas,
             choices = c(1,2), permutations = 0, na.rm=TRUE)
write.csv(round(cbind(ef$vectors$arrows, ef$vectors$r),2), paste0("Model-output/db-rda/16S_6mo_variance-by-predictors_rarefied.csv"))
...
rdadat$file.name <- mdatseq_6mo_nonas0$file.names.16S
...
rdadat$Glucose.addition.treatment <- as.factor(rdadat$Glucose.addition.treatment)
levels(rdadat$Glucose.addition.treatment) <- c("No glucose", "Glucose")
summary(myrda_6mo)$cont$importance[2,1] #0.4926851
summary(myrda_6mo)$cont$importance[2,2] #0.02542659
db6mo <- ggplot(rdadat, aes(CAP1, CAP2)) +
  geom_point(aes(shape=Moisture.treatment, color=Residue.type, alpha=Glucose.addition.treatment)) +
  theme_bw() +
  scale_color_manual(values = covercols) +
  scale_alpha_manual(values = c(0.5,1)) +
  labs(x=("RDA 1 (49.2%)"), y="RDA 2 (2.5%)") + # amend according to variance explained
...
db6mo
pdf(paste0("Figures/_communities/db-rda/16S_6mo1_rarefied.pdf"), height=3.8, width=5.5)
db6mo
dev.off()
...
#figure
png("figures/_communities/db-rda/16S_rarefied.png", 
...
save.image(file='model-output/communityanalysis-rdata/3d_communityanalysis_16S_rarefied.RData')
load(file='model-output/communityanalysis-rdata/3d_communityanalysis_16S_rarefied.RData')
...
data.frame(rownames(abundances), mdatseq$file.names.16S)
# samples to remove
...
write.csv(abundances_t, "raw-data/sequence/16S/Abundances by taxon_24hr_rarefied.csv")
...
write.csv(dd, "raw-data/sequence/16S/Lefse_24hr_rarefied.csv")

# export  abundances for lefse taxa 
abundances_t_lefse <- abundances_t[which(rownames(abundances_t) %in% dd$features),]
write.csv(abundances_t_lefse, "raw-data/sequence/16S/Abundances by taxon_24hr_Lefse_rarefied.csv")
...
lefsep <-ggplot(ddgrid1, aes(fill= score)) + 
  ...
  geom_segment(inherit.aes=FALSE, size=0.5, data=segs, aes(x=-6, xend=-6, y=ybar1, yend=ybar2)) +
  annotate("text", x = -6.2, y = segs$ylab+0.5, label = segs$label, size=2.3, hjust=1, vjust=0.5) #+
#theme_minimal()
...
figure
png("figures/_lefse/1_16S_24 hr_rarefied.png", 
...
abundances_t_lefse <- read.csv("raw-data/sequence/16S/Abundances by taxon_24hr_Lefse_rarefied.csv")
...
mdatseq[which(mdatseq$Glucose.addition.treatment=="G-00"),c("X13soc", "X13poc", "X13maoc", "X13doc", "X13mbc", "X13co2", "CUE", "co2.native")] <- mdatseq[which(mdatseq$Glucose.addition.treatment=="G-50"),c("X13soc", "X13poc", "X13maoc", "X13doc", "X13mbc", "X13co2", "CUE", "co2.native")]
# order by sequence file names
mdatseq <- mdatseq[order(mdatseq$file.names.16S),]
mdatseq$X13residue <- mdatseq$X13soc - mdatseq$X13mbc
mdatseq$CSE <- (mdatseq$X13residue + mdatseq$X13mbc)/(mdatseq$X13residue + mdatseq$X13mbc + mdatseq$X13co2)
mdatseq$CSE[which(mdatseq$CSE<0)] <- NA
mdatseq$X13residue[which(mdatseq$X13residue<0)] <- NA
...
abundances_t_lefse_t2 <- abundances_t_lefse_t[rownames(abundances_t_lefse_t) %in% mdatseq_24hr$file.names.16S,]
abundances_t_lefse_t2$file.names.16S <- rownames(abundances_t_lefse_t2)
unique(mdatseq_24hr$file.names.16S==abundances_t_lefse_t2$file.names.16S) #check

# combine abundances and soil property data
newdat <- merge(mdatseq_24hr, abundances_t_lefse_t2, by="file.names.16S",
                all.x = TRUE, all.y = TRUE)

terms.soil <- c(rownames(summary(myrda_24hr)$biplot)) 
terms.taxa <- colnames(abundances_t_lefse_t2)
terms.taxa <- terms.taxa[-which(terms.taxa %in% c("file.names.16S", "file.names"))]  
...
p<-ggplot(ddgrid3, aes(fill= cor)) + 
  ...
  theme(axis.text.x = element_text(angle = 270, vjust = 0, hjust=0, size=12), 
...
figure <- ggpubr::ggarrange(ann2,  p, 
                            ncol = 1, nrow = 2, heights=c(0.025,0.975)) 
#figure
png("figures/_lefse/1_16S_24 hr_correlations_rarefied.png", 
    width = 1000, height =4000, res=300, units="px")
figure
dev.off()
...
abundances_t <- abundances_t4
# export abundances by taxon
write.csv(abundances_t, "raw-data/sequence/16S/Abundances by taxon_6mo_rarefied.csv")
...
# export lefse results 
write.csv(dd, "raw-data/sequence/16S/Lefse_6mo_rarefied.csv")

# export  abundances for lefse taxa 
abundances_t_lefse <- abundances_t[which(rownames(abundances_t) %in% dd$features),]
write.csv(abundances_t_lefse, "raw-data/sequence/16S/Abundances by taxon_6mo_Lefse_rarefied.csv")
...
figure <- ggpubr::ggarrange(ann1, lefsep, legend="top",
                            ncol = 1, nrow = 2, heights=c(0.025,0.975)) 
#figure
png("figures/_lefse/1_16S_6 mo_rarefied.png", 
...
# import  abundances for lefse taxa 
abundances_t_lefse <- read.csv("raw-data/sequence/16S/Abundances by taxon_6mo_Lefse_rarefied.csv")
# order by primary lifestyle
# create columns for taxonomic group
abundances_t_lefse$kingdom <- rep(NA, dim(abundances_t_lefse)[1])
abundances_t_lefse$phylum <- rep(NA, dim(abundances_t_lefse)[1])
...
abundances_t_lefse_t <- as.data.frame(t(abundances_t_lefse))
colnames(abundances_t_lefse_t) <- abundances_t_lefse_t[dim(abundances_t_lefse_t)[1],]
abundances_t_lefse_t2 <- abundances_t_lefse_t[rownames(abundances_t_lefse_t) %in% mdatseq_6mo$file.names.16S,]
abundances_t_lefse_t2$file.names.16S <- rownames(abundances_t_lefse_t2)
unique(mdatseq_6mo$file.names.16S==abundances_t_lefse_t2$file.names.16S) #check

# combine abundances and soil property data
newdat <- merge(mdatseq_6mo, abundances_t_lefse_t2, by="file.names.16S",
                all.x = TRUE, all.y = TRUE)
...
figure <- ggpubr::ggarrange(ann2,  p, 
                            ncol = 1, nrow = 2, heights=c(0.025,0.975)) 
#figure
png("figures/_lefse/1_16S_6 mo_correlations_rarefied.png", 
    width = 900, height =4000, res=300, units="px")
figure
dev.off()
