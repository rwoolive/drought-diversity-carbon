...
# cor(distance,ax1_dist)^2
# ax2_dist <- vegdist(m$points[,2],method="euclidean")
# cor(distance,ax2_dist)^2
# with(mdatseq, levels(Harvest.day2))
# scl <- 2
# # color by residue type
# png(paste0("figures/*communities/nmds/ITS_all_cover.png"), width=600, height=650, res = 150)
...
# legend("topleft", legend=levels(mdatseq_2$Residue.type),fill=covercols, cex=0.8)
# legend("topright", legend=levels(mdatseq_2$Harvest.day2),pch =c(21,23), cex=0.8)
# dev.off()
# 
# 
# # 24 hr
...
# scl <- 2
# # color by residue type
# png(paste0("figures/*communities/nmds/ITS_24hr_res.png"), width=600, height=650, res = 150)
# plot(s$sites, type="n")#, xlim=c(-1,2.5), ylim=c(-1.1,1.5))
...
# scl <- 2
# # color by residue type
# png(paste0("figures/*communities/nmds/ITS_6mo_res.png"), width=600, height=650, res = 150)
# plot(s$sites, type="n")#, xlim=c(-1,2.5), ylim=c(-1.1,1.5))
# with(mdatseq_2, points(s$sites,  scaling =scl, pch =c(21,23)[Moisture.treatment], bg = alpha(covercols[Residue.type],0.7)))
# #with(mdatseq, legend("topright", legend=levels(Cover), bty="n", col="black", pch=21, pt.bg=alpha(covercols,0.9)))
# ordiellipse(m, group=mdatseq$Residue.type, col = alpha(covercols, 0.9), label=F, draw="polygon", border = NA )
# legend("topleft", legend=levels(mdatseq_2$Residue.type),fill=covercols, cex=0.8)
# legend("topright", legend=levels(mdatseq_2$Moisture.treatment),pch =c(21,23), cex=0.8)
# dev.off()
...
write.csv(permmod2, paste0("Model-output/permanova/with glucose_ITS by timepoint_24 hr.csv"))
...
permmod <- adonis2(divmat_pa_6mo ~ Moisture.treatment*Residue.type*Glucose.addition.treatment, 
                   ...
                   by = "terms" )
permmod2 <- round(as.data.frame(permmod), 3)
write.csv(permmod2, paste0("Model-output/permanova/with glucose_ITS by timepoint_6 mo.csv"))
...
write.csv(mod.effects, paste0("Model-output/permanova/ITS by timepoint.csv"))
...
myrda_24hr <- capscale(formula = divmat_pa_24hr ~ X13soc + X13co2+ pom.cn+ maom.cn+ co2 + bulk.soil.C.g.kg,
                       distance = "bray", 
                        data = mdatseq_24hr, na.action = "na.omit")
...
write.csv(as.data.frame(summary(myrda_24hr)$biplot), paste0("Model-output/db-rda/ITS_24hr_loadings.csv"))
# site coordinates
write.csv(as.data.frame(summary(myrda_24hr)$site), paste0("Model-output/db-rda/ITS_24hr_axes.csv"))
# variation explained by first two axes
write.csv(summary(myrda_24hr)$cont$importance, paste0("Model-output/db-rda/ITS_24hr_variance-explained.csv"))
# variation explained by each soil property
mdatseq_24hr_nonas0 <- na.omit(mdatseq_24hr[,c("Residue.type", "Moisture.treatment", "Glucose.addition.treatment", 
...
write.csv(round(cbind(ef$vectors$arrows, ef$vectors$r),2), paste0("Model-output/db-rda/ITS_24hr_variance-by-predictors.csv"))
...
summary(myrda_24hr)$cont$importance[2,1] #
summary(myrda_24hr)$cont$importance[2,2] #
db24hr <- ggplot(rdadat, aes(CAP1, CAP2)) +
  ...
  labs(x=("RDA 1 (48.1%)"), y="RDA 2 (6.7%)") + # amend according to variance explained
  lims(x=c(-1.2,1.5)) + 
  ...
  theme()
db24hr
pdf(paste0("figures/*communities/db-rda/ITS_24hr1.pdf"), height=3.75, width=5.5)
db24hr
dev.off()
...
myrda_6mo <- capscale(formula = divmat_pa_6mo ~ co2 + bulk.soil.C.g.kg + X13maoc+ maom.cn,
                      distance = "bray", 
                       data = mdatseq_6mo, na.action = "na.omit")
...
# export mod effects
write.csv(t(mod.effects), paste0("Model-output/db-rda/ITS_dbrda.csv"))

## rda all
# factors associated with each axis
write.csv(as.data.frame(summary(myrda_6mo)$biplot), paste0("Model-output/db-rda/ITS_6mo_loadings.csv"))
# site coordinates
write.csv(as.data.frame(summary(myrda_6mo)$site), paste0("Model-output/db-rda/ITS_6mo_axes.csv"))
# variation explained by first two axes
write.csv(summary(myrda_6mo)$cont$importance, paste0("Model-output/db-rda/ITS_6mo_variance-explained.csv"))
# variation explained by each soil property
...
ef <- envfit(myrda_6mo, mdatseq_6mo_nonas,
             choices = c(1,2), permutations = 0, na.rm=TRUE)
write.csv(round(cbind(ef$vectors$arrows, ef$vectors$r),2), paste0("Model-output/db-rda/ITS_6mo_variance-by-predictors.csv"))
...
rdadat$Glucose.addition.treatment <- as.factor(rdadat$Glucose.addition.treatment)
levels(rdadat$Glucose.addition.treatment) <- c("No glucose", "Glucose")
summary(myrda_6mo)$cont$importance[2,1] #
summary(myrda_6mo)$cont$importance[2,2] #
db6mo <- ggplot(rdadat, aes(CAP1, CAP2)) +
  geom_point(aes(shape=Moisture.treatment, color=Residue.type, alpha=Glucose.addition.treatment)) +
  theme_bw() +
  scale_color_manual(values = covercols) +
  scale_alpha_manual(values = c(0.5,1)) +
  labs(x=("RDA 1 (52.2%)"), y="RDA 2 (6.6%)") + # amend according to variance explained
...
db6mo
pdf(paste0("figures/*communities/db-rda/ITS_6mo1.pdf"), height=3.75, width=5.5)
db6mo
dev.off()
...
figure
png("figures/*communities/db-rda/ITS.png", 
...
for(i in 1:dim(dd)[1]){
  split_name <- strsplit(dd$Names[i], "[|]")
...
write.csv(dd, "raw-data/sequence/ITS/Lefse_24hr.csv")

# export  abundances for lefse taxa 
abundances_t_lefse <- abundances_t[which(rownames(abundances_t) %in% dd$Names),]
write.csv(abundances_t_lefse, "raw-data/sequence/ITS/Abundances by taxon_24hr_Lefse.csv")
...
lefsep <-ggplot(ddgrid1, aes(fill= score)) + 
  ...
  geom_segment(inherit.aes=FALSE, size=1, data=segs, aes(x=-7, xend=-7, y=ybar1, yend=ybar2)) +
  annotate("text", x = -7.3, y = segs$ylab+0.2, label = segs$label, size=2, hjust=1, vjust=0.5) #+
  #theme_minimal()
...
lefsep
  ...
  figure
  png("figures/*lefse/1_ITS_24 hr.png", 
      width = 1000, height =2500, res=300, units="px")
  figure
  dev.off()
...
#### 4. Pearson correlations: relationships between soil properties 
  ...
  abundances_t_lefse <- read.csv("raw-data/sequence/ITS/Abundances by taxon_24hr_Lefse.csv")
...
figure <- ggpubr::ggarrange(ann2,  p, 
                            ncol = 1, nrow = 2, heights=c(0.1,0.9)) 
figure
png("figures/*lefse/1_ITS_24 hr_correlations.png", 
    width = 500, height =2500, res=300, units="px")
figure
dev.off()
...
abundances_t <- abundances_t4
# export abundances by taxon
write.csv(abundances_t, "raw-data/sequence/ITS/Abundances by taxon_6mo.csv")
...
# export lefse results 
write.csv(dd, "raw-data/sequence/ITS/Lefse_6mo.csv")

# export  abundances for lefse taxa 
abundances_t_lefse <- abundances_t[which(rownames(abundances_t) %in% dd$Names),]
write.csv(abundances_t_lefse, "raw-data/sequence/ITS/Abundances by taxon_6mo_Lefse.csv")
...
ddgrid$primarylifestyle[which(ddgrid$primarylifestyle=="NA")] <- "unspecified"
ddgrid$primarylifestyle <- as.factor(ddgrid$primarylifestyle)
ddgrid$primarylifestyle <- factor(ddgrid$primarylifestyle, levels(ddgrid$primarylifestyle)[rev(c(2,4,7,9,10,1,3,5,6,8))])
...
figure <- ggpubr::ggarrange(ann1, lefsep, legend="top",
                            ncol = 1, nrow = 2, heights=c(0.1,0.9)) 
figure
png("figures/*lefse/1_ITS_6 mo.png", 
    width = 1000, height =2500, res=300, units="px")
figure
dev.off()
...
# import  abundances for lefse taxa 
abundances_t_lefse <- read.csv("raw-data/sequence/ITS/Abundances by taxon_6mo_Lefse.csv")
# order by primary lifestyle
# create columns for taxonomic group
abundances_t_lefse$kingdom <- rep(NA, dim(abundances_t_lefse)[1])
abundances_t_lefse$phylum <- rep(NA, dim(abundances_t_lefse)[1])
...
ddgrid2$primarylifestyle[which(ddgrid2$primarylifestyle=="NA")] <- "unspecified"
ddgrid2$primarylifestyle <- as.factor(ddgrid2$primarylifestyle)
ddgrid2$primarylifestyle <- factor(ddgrid2$primarylifestyle, levels(ddgrid2$primarylifestyle)[rev(c(2,4,7,9,10,1,3,5,6,8))])
ddgrid3 <- ddgrid2[order(ddgrid2$primarylifestyle, ddgrid2$taxon),]
ddgrid3$taxon <- as.factor(ddgrid3$taxon)
ddgrid3$taxon <- factor(ddgrid3$taxon, unique(ddgrid3$taxon))
...
figure <- ggpubr::ggarrange(ann2,  p, 
                            ncol = 1, nrow = 2, heights=c(0.1,0.9)) 
figure
png("figures/*lefse/1_ITS_6 mo_correlations.png", 
    width = 400, height =2500, res=300, units="px")
figure
dev.off()
