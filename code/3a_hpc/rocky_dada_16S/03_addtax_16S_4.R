...
seqtab.nochim <- "rocky_dada_16S/output/02_nochimera_mergeruns/rocky_dada_16S/rocky_dada_16S_seqtab_final.rds" #change to your file structure
output_16S <- "rocky_dada_16S/output"
name <- "rocky_dada_16S"
tax_db <- "rocky_dada_16S/rdp_train_set_18.fa.gz" #change to your tax database (Silva, UNITE, MaarjAM*)
method <- "dada"
...
seqtab.nochim <- seqtab.nochim[,600001:800000]
#seqtab.nochim <- seqtab.nochim[,807456:807459] # test
...
num_step = 100
#num_step = 3 # test
seq_per_step = as.integer(num_seq[2] / num_step)
...
last_step_num_seq = num_seq[2] %% num_step

taxa_0 = data.frame(Kingdom=NA,Phylum=NA,Class=NA,Order=NA,Family=NA,Genus=NA)#,Species=NA)
for(i in 1:num_step){
  sbegin = 1 + (i-1)*seq_per_step
  send = i*seq_per_step
  print(paste("Processing sequences", sbegin, "through", send, "of", num_seq[2]))
  taxa_s <- assignTaxonomy(colnames(seqtab.nochim)[sbegin:send], tax_db, multithread=TRUE, tryRC = TRUE)
...
# Write to disk
saveRDS(taxa_0, paste0(output_16S, name, "_tax_assignation_4.rds"))

cat('\n')
cat(paste0('# The obtained taxonomy file can be found in "', paste0(output_16S, name, "_tax_assignation_4.rds"), '"\n'))
cat('\n# All done!\n\n')
