...
output2 <- "rocky_dada_16S/output" # where we will dump the results//diagnostic
name.run <- "rocky_dada_16S" # Run name. A directory for the values will be created
tlens <- as.integer(c(250,210)) #how long to truncate forward and reverse reads
maxee <- as.integer(c(2,5))

minover <- as.integer(c(15)) ##Default value is 15, not a good option go below 10.
...
### filter and trim
#forprim <- "CCTACGGGNGGCWGCAG" # forward primer 341F
#revprim <- "GACTACHVGGGTATCTAATCC" # reverse primer 785R
...
cat(paste0('# bp and max expected error ',maxee[1],'\n'))
cat(paste0('# bp and max expected error ',maxee[2],'\n\n'))

## ------------------------------------------------------------------------
#Check and create a dir for output results

dir.create(file.path(output2, "01_errors-output"), showWarnings = FALSE)
dir.create(file.path(output2, "01_errors-output", name.run), showWarnings = FALSE)

output2 <- str_c(output2,"/01_errors-output/",name.run,"/")
...
ggsave(str_c(output2,"errors_",name.run,"_fwd.pdf"),plot=err.plotf, width = 9, height = 8)
err.plotr <- plotErrors(errR, nominalQ=TRUE)
ggsave(str_c(output2,"errors_",name.run,"_rev.pdf"),plot=err.plotr, width = 9, height = 8)
...
mergers <- mergePairs(dadaFs, derepFs,
                      dadaRs, derepRs,
                      minOverlap = minover, justConcatenate=TRUE)
...
saveRDS(seqtab, str_c(output2,name.run,"_seqtab.rds"))
## ------------------------------------------------------------------------
...
write_tsv(data.frame(track),str_c(output2,name.run,"_track_analysis.tsv"))
...
cat(paste0('\n# An ASV table was created, you can find in "',str_c(output2,name.run,"_seqtab.rds"),'"\n'))
...
cat(paste0('# In "',str_c(output2,name.run,"_track_analysis.tsv"),"\" you will find a table where you can check the loss of reads in each step. Check it out to see if everything's correct!",'\n'))
cat('\n# All done!\n\n')
